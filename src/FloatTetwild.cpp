...
void extract_volume_mesh2(const Mesh& mesh, MatrixXs& V, Eigen::MatrixXi& T)
{
                       ...
                       bool             skip_simplify)
{
    ...
    // timer.start();
    // optimization(input_vertices, input_faces, input_tags, is_face_inserted, mesh, tree, { {1, 1,
    // 1, 1} }); logger().info("mesh optimization {}s", timer.getElapsedTimeInSec());
    // logger().info("");
